...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "analysis_results" / "average_citations"


def load_average_citations_function():
    module_path = PROJECT_ROOT / "functions" / "get_averagecitations.py"
    spec = importlib.util.spec_from_file_location("get_averagecitations", module_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.get_average_citations


def write_summary(table, output_file):
    total_documents = int(table["N"].sum())
    peak = table.sort_values(["MeanTCperYear", "Year"], ascending=[False, True]).iloc[0]
    highest_mean = table.sort_values(["MeanTCperArt", "Year"], ascending=[False, True]).iloc[0]

    lines = [
        "Average Citations Analysis",
        f"Documents with valid year: {total_documents}",
        f"Year range: {int(table['Year'].min())}-{int(table['Year'].max())}",
        f"Highest mean citations/year: {int(peak['Year'])} ({peak['MeanTCperYear']})",
        f"Highest mean citations/article: {int(highest_mean['Year'])} ({highest_mean['MeanTCperArt']})",
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the ETL chain and then get_average_citations."
    )
    parser.add_argument(
        "--input",
    ...
    get_average_citations = load_average_citations_function()
    fig, table = get_average_citations(data)

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    csv_path = output_dir / "average_citations_table.csv"
    html_path = output_dir / "average_citations_plot.html"
    summary_path = output_dir / "average_citations_summary.txt"
